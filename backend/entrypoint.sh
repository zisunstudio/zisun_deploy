...
# Pause switch for the background processes, for docker compose. On Railway
# this script never runs for worker/beat - a startCommand bypasses the
# ENTRYPOINT - so the same switch lives in app/celery_app.py at import time.
