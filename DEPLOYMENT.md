...
# Worker/beat only: 1 = do not start Celery, sleep instead. Set while the
# broker is unusable (a spent Upstash quota); unset once Redis is healthy.
# CELERY_PAUSED=1

# Console AI (product drafting by voice/text, the daily brief). Optional:
# unset = those features say they are unavailable, nothing else changes.
...
| `NEXT_PUBLIC_WHATSAPP_NUMBER` | Digits with country code, e.g. `919363608792`. Optional: the storefront falls back to `COMPANY.phone` in `src/lib/legal.ts`, so the direct chat exists without it |
| `NEXT_PUBLIC_WHATSAPP_GROUP_URL` | `https://chat.whatsapp.com/…` — the ZISUN Tales group; only used when no number can be found at all |
