...
import { useProducts, useCategories, SortBy, type Category, type ProductListResponse } from "@/lib/queries/catalog";
...
export interface ShopInitial { products?: ProductListResponse; categories?: Category[] }

/** The interactive collection, inside the server shell in page.tsx. */
export default function ShopView({ initial = {} }: { initial?: ShopInitial }) {
  const router = useRouter();
  const [categoryId, setCategoryId] = useState<string | undefined>();
  const [sortBy, setSortBy] = useState<SortBy>("newest");
  const [showFilters, setShowFilters] = useState(false);

  // Server data only seeds the default view - the one the server fetched.
