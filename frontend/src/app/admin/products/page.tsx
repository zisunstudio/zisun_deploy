...
type Product = { id: string; name: string; base_price: number; is_active: boolean; variants: Variant[]; category?: { name: string } };
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                {["Name", "Category", "Base Price", "Variants", "Status", "Actions"].map((h) => (
            ...
            <tbody className="divide-y divide-gray-100">
              {(products ?? []).map((p) => (
                <tr key={p.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-600">{p.variants.length} variants</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      <button
                        onClick={() => { setEditId(p.id); setEditName(p.name); setEditPrice(String(p.base_price)); }}
          ...
          <div className="bg-white rounded-xl p-6 w-80 shadow-xl">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input className="w-full border rounded-lg px-3 py-2 text-sm mt-1" type="number" value={editPrice} onChange={(e) => setEditPrice(e.target.value)} />
              </div>
            </div>
            <div className="flex gap-2 mt-4">
              <button
                onClick={() => updateProduct.mutate({ id: editId, data: { name: editName, base_price: parseInt(editPrice) } })}
                className="flex-1 bg-[#5C3317] text-white py-2 rounded-lg text-sm font-semibold"
              >Save</button>
